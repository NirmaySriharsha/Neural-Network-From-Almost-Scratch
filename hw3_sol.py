...
def define_network(size_hidden,
    ...
    #model = MultiLayerPerceptron([
    #    Linear(init_linear_params(size_input, 384)), 
    #    ReLU(),
    #    Linear(init_linear_params(384, 256)),
    #    ReLU(),
    #    Linear(init_linear_params(256, size_output)) 
    #])
...
def train_network(model, dataset, num_epoch,
    ...
    for idx_epoch in range(num_epoch):  # for each epoch
        ...
        for slice_start in range(0, len(train_x), batch_size):
            ...
            y = train_y[slice_start: slice_start + batch_size]
            y_pred = model.forward(x)
            training_loss_this_epoch += criterion(y_pred, y)
            n = n + 1
        training_loss_this_epoch = training_loss_this_epoch/n
        ...
        overall_loss = criterion(y_pred, train_y)
        yhat_dists = criterion.y_hat
        yhat_train_all = yhat_dists.argmax(axis = 1)
        ...
        n = 0
        for slice_start in range(0, len(test_x), batch_size):
            x = test_x[slice_start: slice_start + batch_size, :]
            y = test_y[slice_start: slice_start + batch_size]
            y_pred = model.forward(x)
            testing_loss_this_epoch += criterion(y_pred, y)
            n = n + 1
        testing_loss_this_epoch = testing_loss_this_epoch/n
        ...
        overall_loss = criterion(y_pred, test_y)
        yhat_dists = criterion.y_hat
        yhat_test_all = yhat_dists.argmax(axis = 1)
...
model = define_network(256)
dataset = load_data()
result = train_network(model, dataset, 50, 0.01, 200)
print(result['train_accuracy'][-1])
